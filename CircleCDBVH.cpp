...
struct Triangle
{
	xy pt[3]; //삼각형은 세개의 꼭지점이 필요하니 한쌍의 좌표가 세개 필요하다.
};
std::vector<Triangle> tris; //삼각형 벡터 
bool IsCollision(Ball b0, Ball b1); //바운딩 볼륨끼리 서로 충돌하는지를 테스트?
bool IsCollision(Triangle t0, Triangle t1); //삼각형 끼리 충돌하는지를 테스트 
...
public:
	...
	std::vector<std::pair<Triangle, Triangle>> GetCollisions(); //pair는 데이터쌍
};
...
std::vector<std::pair<Triangle, Triangle>> BallBVTT::GetCollisions()
{
	...
	while(!q.empty())
	{
		auto pair = q.front(); //auto 는 변수 추론
		q.pop();
...
BallBV::BallBV(std::vector<Triangle> inputData)
{
	...
	if(tris.size() > 2)
	{
		...
		double xMid = (xMin + xMax) / 2.0; //x값만 가지고 child 나눔
		for(auto& tri : tris)
		{
			if(tri.pt[0][0] < xMid)
